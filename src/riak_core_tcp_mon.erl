...
ssl_test_() ->
    ...
    {timeout, 60, fun() ->
        ...
        {ok, LS} = ssl:listen(0, [{active, true}, binary, {verify, verify_none}, {certfile, "test/site1-cert.pem"}, {keyfile, "test/site1-key.pem"}]),
        ...
        {ok, Socket} = ssl:connect("localhost", Port, [binary, {active, true}, {verify, verify_none}, {certfile, "test/site2-cert.pem"}, {keyfile, "test/site2-key.pem"}]),
